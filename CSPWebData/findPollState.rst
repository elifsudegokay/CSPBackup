...
Belli Bir Eyalet İçin Kirlilik Değerini Bulmak
===================================

Verilerimizi ":" ile ayrılmış alanlara göre ayırabilmek için **split** kullanabiliriz. Ayrıca verileri aralarındaki boşluklara göre de **split** ile gruplandırabiliriz.
Ancak alanı * ayırabilmek için * alanı içinde de kullanabiliriz.Eğer şehir/eyalet bilgisine boşluklara göre split işlemi yaparak şehir ve eyaleti listenin iki farklı elemanı olarak elde ederiz.
...
.. activecode:: splitCityState
   ...
   satir = "Yuma, AZ :14 :9"
   veriler = satir.split(":")
   print("Satırdaki tüm veriler:")
   print(veriler)
   sehirEyalet = veriler[0].split(" ")
   print("Şehir:", sehirEyalet[0])
   print("Eyalet:", sehirEyalet[1])
   print("PM 10:", sehirEyalet[1])
   print("PM 2.5:", sehirEyalet[2])

Eğer şehir isminde boşluk olsaydı ikinci veri olarak eyalet ismini alamazdık. Böyle durumlarda ``,`` 'e göre ayırmak daha güvenlidir.
...
Peki sadece belli bir eyaletin verilerine ihtiyaç duyduğumuzu varsayalım. Bu durumda verisini istediğimiz eyaleti mevcut eyaletlerde aramamız gerekirdi. ``veriSeti.find(eyaletİsmi)`` işlevi ile aradığımız eyaletin veri setinde olup olmadığını kontrol edebiliriz. ``find`` işlevi ``eyaletİsmi`` değerini ``veriSeti`` içerisinde arayacak bulduğu taktirde bulduğu yerdeki index numarasını döndürecek, bulamadğında ise -1 değerini döndürecektir.
...
.. activecode:: average25state
   ...
   for satir in satirlar:
       ...
       sehirEyalet = degerler[0].split(",")
       
       
       if sehirEyalet[1].find(eyalet) >= 0:
       
           
           yeni25 = float(degerler[2]) 
           toplam25 = toplam25 + yeni25
           
           
           sayac = sayac + 1
	 
   
   print("Ortalama PM 2.5 değeri " , eyalet, " eyaleti için ", toplam25 / sayac)

Aşağıdaki yorum satırlarını (``#``) kaldırarak PM 10 değerinin ortalamasını verecek gerekli kodları tamamlayın.
...
.. activecode:: average10state
   ...
   eyalet = "CA"
   # toplam10 değişkenini yaratın ve ilk değerini verin
   sayac = 1.0
   for satir in satirlar:
       degerler = satir.split(":")
       sehirEyalet = degerler[0].split(",")
       if sehirEyalet[1].find(eyalet) >= 0:
           # PM 10 kirlilik değerini al
           # aldığın değeri toplam10 değişkenine ekle
           sayac = sayac + 1
	 
   print("Ortalama PM 10 değeri ", eyalet, " eyaleti için ", total10 /sayac)

Hangi eyaletlerin ortalama kirlilik değeri en fazladır? Kirliliğin artmasında nüfus, bölge ya da refah seviyesi ne kadar etkilidir ? Bu bilgiler PM 2.5 ve PM 10 açısından farklılıklar gösterir mi ? Bilgi için : http://www.50states.com/abbreviations.htm#.VJCB9r5NsXc.
