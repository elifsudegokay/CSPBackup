...
Ortalama PM 2.5 kirlilik miktarı nedir?
======================================

Bu defa, her bir PM 2.5 değerini alıyoruz ve hepsini topluyoruz, bu sırada her bir değer için bir sayaçı bir arttırıyoruz ki ortalamaya hesaplayabilelim.
Bu bölüm de, önceki bölümlerde kullandığımız veri dosyasını kullanıyoruz, eğer **tüm** verileri görmek istiyorsanız aşağıdaki *Göster* butonuna tıklayabilirsiniz. Eğer saklamak isterseniz *Gizle* butonuna tıklayabilirsiniz.
...
.. activecode:: average25
   ...
   # değişkenlerin başlangıç değerlerini koy
   toplam25 = 0
   sayac = 1.0
   ...
   for satir in satirlar:
       ...
       # bir satırı ":" sembolüne göre parçala
       degerler = satir.split(":")
       
       # PM 2.5 kirlilik değerini al
       yeni25 = float(degerler[2])
       
       # toplam miktarını ve sayacı güncelle
       toplam25 = toplam25 + yeni25
       sayac = sayac + 1
	 
   # PM 2.5 kirlilik değerini bastır
   print("Ortalama PM 2.5 değeri ",toplam25/sayac)

Şimdi, aşağıda sadece  ``#``  sembolü ile başlayan satırları, ortalama PM 10 değerini hesaplamak için değiştirin.
...
.. activecode:: average10
   ...
   # toplam10 değişkenini yarat ve başlangıç değerini ata
   sayac = 1.0
   for satir in satirlar:
       values = satir.split(":")
       # şu anki satırdan PM 10 değerini alın
       # aldığınız PM 10 değerini toplam10 değişkenine ekleyin
       sayac = sayac + 1
	 
   print("Ortalama PM 10 değeri ", toplam10 / sayac)
   
.. parsonsprob:: 18_6_1_avg25

   Sıradaki program maksimum PM 2.5 kirlilik değerini bastırıyor ancak verilen kodun sırası karıştırılmış. Kod bloklarını sürükleyerek, doldaki kolondan sağdaki kolona doğru sırayla yerleştiriniz. Sonra Check me butonuna tıklayarak doğru yaptınız kontrol ediniz. Eğer satırların sırası hala yanlış sıradaysa ya da girintileme hatası yaptıysanız uyarı mesajı alacaksınız. 
   -----
   # bütün satırları oku
   dosya = open("uspoll.txt","r")
   satirlar = dosya.readlines()
