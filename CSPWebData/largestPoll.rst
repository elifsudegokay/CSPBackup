...
En yüksek kirlilik değeri nedir?
======================================

Bu bölümde, daha önce kullandığımız aynı veri dosyasını kullanıyoruz; ancak **tüm** verileri görmek isterseniz aşağıdaki Göster butonuna tıklayabilirsiniz. Dosyayı görüntülediğinizde, Gizle butonuna tıklayarak tekrar gizleyebilirsiniz.
...
Araştırılması gereken ilginç sorulardan biri şudur: “Hangi şehir, en kötü kirliliğe sahiptir?”. Aşağıdaki program, PM 2.5 değeri için olanı hesaplar. Program tüm veriyi okuyacak ve ardından ``max25`` değerini (maksimum PM 2.5 değeri) sıfıra ayarlayacaktır. Daha sonra tüm şehir verileri üzerinden döngüleyecek ve mevcut kirlilik ``(yeni25)`` değeri ``max25`` değerinden büyükse, bu değeri ``max25`` değeri olarak ve bu şehir adını da ``maxSehir`` olarak kaydedecektir. Dizgiyi (string), karşılaştırabileceğimiz bir sayıya dönüştürmek için ``float`` fonksiyonuna (işlevine) ihtiyacımız var.
...
.. activecode:: max25
   ...
   # değişkenlere başlangıç değerlerini ver
   maksSehir = ''
   maks25 = 0 
   
   # tüm satırları döngüle
   for satir in satirlar:
   
       # satırı “:” sembolünde ayır 
       degerler = satir.split(":")
       
       # PM 2.5 kirlilik değerini al
       yeni25 = float(degerler[2])
       
       # eğer mevcut değer maksimum değerden büyükse
       if yeni25 > maks25:
       
           # yeni maksimum bilgiyi kaydet
           maksSehir = degerler[0]
           maks25 = yeni25 # yeni maksimum değeri kaydet
           
   # en büyük PM 2.5 değerini yazdır
   print("En yüksek değer; ",maksSehir," şehrindeki ", maks25," değeridir.")
   
.. mchoice:: 18_4_3_max10
   :answer_a: yeni10 = float(degerler[0])
   :answer_b: yeni10 = float(degerler[1])
   :answer_c: yeni10 = float(degerler[2])
   :correct: b
   :feedback_a: Bu kod satırı, şehir ve eyaleti alacaktır.
   :feedback_b: Bu kod satırı, PM 10 değerini alır.
   :feedback_c: Bu kod satırı, PM 2.5 değerini alır. 

   Aşağıdakilerden hangisi, 17. satırdaki kodda belirtilen değerin yerine PM 10 kirliliğinin değerini döndürür?  
   

   
En yüksek PM 10 kirliliğine sahip olan şehri bulmak için; aşağıdaki kodda, yorum sembolü (``#``) ile başlayan satırları değiştirin. .  
...
.. activecode:: max10
   ...
   maksSehir = ''
   # maks10 değişkenini tanımla ve bu değişkene başlangıç değeri ver
   for satir in satirlar:
       degerler = satir.split(":")
       # yeni10 değişkeninin değerini mevcut PM 10 değeri olarak ayarla
       if yeni10 > maks10:
           maksSehir = degerler[0]
           # yeni maksimum değeri kaydet
   print("En yüksek PM 10 değeri; ", maksSehir," şehrindeki ", maks10, " değeridir.")
           
   
.. mchoice:: 18_4_3_max10_max25_q
   :answer_a: Aynı şehir hem PM 2.5 hem de PM 10 için maksimum değere sahiptir.
   :answer_b: Maksimum değerler farklı şehirlere aittir, fakat bu şehirler aynı eyalettedirler.
   :answer_c: Farklı eyaletlerdeki farklı şehirler maksimum değerlere sahiptir.
   :correct: a
   :feedback_a: Fresno,CA her ikisi için de en kötü değere sahiptir.
   :feedback_b: En kötü PM 10 değerlerini bulmak için degerler[1] kullanarak kontrol edin.
   :feedback_c: You did not actually run and change the program, did you?

   Maksimum PM 2.5 ve PM 10 değerlerine sahip olan şehirler için aşağıdakilerden hangisi doğrudur?
   
.. parsonsprob:: 18_4_1_maxPM25

   Aşağıdaki program, bulunan maksimum PM 2.5 kirlilik değerini yazdırır ancak kodlar karışık sırada verilmiştir. Kod bloklarını üstteki sütundan alttaki sütuna sürükleyin ve doğru sıraya göre yerleştirin. Sonra Check Me butonuna tıklayarak doğru şekilde yapıp yapmadığınızı kontrol edin. Satırlardan herhangi birisi yanlış sırada ise ya da yanlış girintiye sahip ise ekranda bir uyarı mesajı göreceksiniz.
   -----
   # bütün satırları oku
   dosya = open("uspoll.txt","r")
   satirlar = dosya.readlines()
