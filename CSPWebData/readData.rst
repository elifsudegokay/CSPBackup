...
Bu bölüm az önce gördüğümüz veri dosyasını kullanıyor. **Tüm** verileri görmek istiyorsanız, aşağıdaki *Göster* düğmesine tıklayın. Görüntülendiğinde, *Gizle* düğmesini tıklatarak tekrar gizleyebilirsiniz..
...
`Göster` düğmesine tıkladığınızda çok fazla veri olduğunu söyleyebilirsiniz. Ancak aslında dosyadaki veri satırlarının sayısını inceleyerek bilmiyoruz. Son bir veri kalmayana kadar *while* döngüsü ile veriyi okuyacağız (bos bir karakter dizisi döndürür.)
...
Verileri içeren dosyayı *açıp(opening)* sonra her satırı *okuyarak (reading)* verileri işleyebiliriz. Satır boş olmadığı sürece, bir ayırma ``split`` yapmaya ve verileri işlemeye devam ediyoruz. (Bu veri,  ``,`` yerine ``:`` ile ayrılmıştır.)
...
Python'da, dosyaları kullanmadan önce dosyaları **açmalı** ve onlarla işimiz bittiğinde dosyaları **kapatmalıyız**. Bir dosyayı açmak, daha önce gördüğümüz Turtle nesneleri gibi önceden tanımlanmış işlevlere ve prosedürlere sahip bir Python nesnesini döndürür.
...
Tablo 1, dosyaları açmak ve kapatmak için kullanılabilecek işlevleri ve prosedürleri göstermektedir.

.. _filemethods1a:

===================== ======================== =====================================================
**Metod İsmi**        **Kullanım Şekli**         **Açıklama**
===================== ======================== =====================================================
``open``               ``open(filename,'r')``    dosyaismi ile adlandırılan dosyayı okumak için açar. Dosya nesnesine bir referans döndürür
``open``               ``open(filename,'w')``    dosyaismi ile adlandırılan dosyayı yazmak için açar. Dosya nesnesine bir referans döndürür.
``close``              ``filename.close()``      İşiniz bittikten sonra dosyayı kapatır. 

===================== ======================== =====================================================

Aşağıdaki kod, ``"A"`` ile başlayan tüm şehirler için kirlilik bilgilerini basacaktır. Şehir ve eyalet bilgisiyle, bu şehir için iki kirlilik değerini ayrı bir satırda yazdıracaktır. Kod, dosyadan okunacak başka bir satır kalmayana kadar döngü içinde kalacaktır. (boş bir karakter dizisi döndüğünde duracaktır.)
...
.. activecode:: printData
   ...
   # dosyayı okumak için ac
   dosya = open("uspoll.txt","r")
   
   # dosyadan bir satır oku
   satir = dosya.readline()
   
   # diger bir satir oldugu surece
   while satir:
   
       #  : ayraclarinda veriyi ayir
       degerler = satir.split(":")
       
       # listeden sehir ismini al
       sehir = degerler[0]
       
       # Eger sehir ismi “A” ile baslıyorsa bilgiyi yazdir 
       if (sehir.find("A") == 0):
           print('Sehir: ', sehir)
           print("Kirlilik degerleri:",degerler[1],degerler[2])
...
.. parsonsprob:: 18_3_1_printInfoForCity
   ...
   Sıradaki program ismi ``D`` ile başlayan şehirlerdeki kirlilik miktarını yazdırır, ama karışık olarak verilmiş.  Blokları sola doğru sürükleyin ve sağdaki sıraya yerleştirin. Döngünün gövdesindeki blokları girintilemeyi unutmayın. Girintiyi kaldırmak için bloğu sağa sürükleyin. Çözümünüzü kontrol etmek için Check Me düğmesini tıklayın.
   ...
   =====
        ...
        sehir = v[0]
   =====
        # Eger sehir ismi “D” ile baslıyorsa bilgiyi yazdir 
        if (sehir.find("D") == 0):
            print('Sehir: ', sehir)
            print("Kirlilik degerleri:",v[1],v[2])
   ===== 
       # gelecek satiri oku
       satir = dosya.readline()
   =====
...
Aslında bir dosyada okumak için bir **for** döngüsü kullanmanın bir yolu var. Her şeyi tek bir dev listeye okuyabilir ve ardından listedeki her satırı işlemek için bir **for** döngüsü kullanabiliriz.
...
.. activecode:: printData_withLines
   ...
   # satirlar listesinden dongu kur
...
.. mchoice:: 18_3_1_for_disadvantage
   :answer_a: Büyük bir veri dosyası için bu hafızaya aşırı yüklenebilir.
   :answer_b: for döngüleri verimsizdir. 
   :answer_c: for döngüleri while döngüleri kadar esnek değildir.   
   :correct: a
   :feedback_a: Doğru. Bu çok büyük veri setleri için geçerli olacak.
   :feedback_b: Yanlış. for ve while döngüleri aynı verimliliktedir.
   :feedback_c: Yanlış. while döngüleri esnekken, for döngüleri daha anlaşılabilir olduğundan, sizin için en mantıklı olanı kullanmalısınız

    ``for`` döngüsü kullanmanın, ikinci kodda ki dezavantajı nedir? 

.. tabbed:: 18_3_2_WSt

        .. tab:: Soru

           “uspoll.txt” dosyasını okuyacak bir kod yazın. PM 10 kirlilik değerleri 20 ve üstü olan şehir ve kirlilik değerlerini yazdırın.
        ...
        .. tab:: Cevap
          ...
          .. activecode::  18_3_2_WSa
              ...
              while satir:
                     ...
                     degerler = satir.split(":")
       		     # PM 10 kirlilik verisini al
           	     kirlilik = float(degerler[1])
       		     if (kirlilik > 19):
                            print('Sehir: ', degerler[0])
              		    print("Kirlilik degerleri:",degerler[1],degerler[2])
                     satir = dosya.readline()
